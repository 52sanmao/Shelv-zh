...
struct RecapView: View {
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 0) {
                if !recapEnabled {
                    ...
                    emptyStateView(
                        ...
                        message: tr("Enable at least one period in Settings.", "Aktiviere mindestens eine Periode in den Einstellungen.", "请在设置中至少启用一个周期。")
                    )
                } else {
                    ...
                    if filteredEntries.isEmpty {
                        emptyStateView(
                            icon: "clock",
                            message: tr("No recap generated yet for this period.", "Noch kein Recap für diese Periode erstellt.", "该时段暂无已生成的回顾。")
                        )
                    } else {
                        List {
            ...
            .navigationTitle(tr("Recap", "Recap", "年度回顾"))
    ...
    private func recapRow(_ entry: RecapRegistryRecord) -> some View {
        ...
        return HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text(tr("Top \(type.songLimit)", "Top \(type.songLimit)", "前 \(type.songLimit) 首"))
                    .font(.caption)
    ...
    private var disabledStateView: some View {
        VStack(spacing: 12) {
            ...
            Text(tr("Recap is disabled", "Recap ist deaktiviert", "回顾已禁用"))
                .font(.headline)
                .foregroundStyle(.secondary)
            Text(tr("Enable Recap in Settings to start tracking your listening history.", "Aktiviere Recap in den Einstellungen, um dein Hörverhalten aufzuzeichnen.", "在设置中启用回顾以开始记录您的收听历史。"))
                .font(.subheadline)
                .foregroundStyle(.secondary)
...
extension RecapPeriod.PeriodType {
    var label: String {
        switch self {
        case .week:  return tr("Weekly", "Wöchentlich", "每周")
        case .month: return tr("Monthly", "Monatlich", "每月")
        case .year:  return tr("Yearly", "Jährlich", "每年")
        }
    }
