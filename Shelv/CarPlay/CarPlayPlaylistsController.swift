...
final class CarPlayPlaylistsController {
    ...
    init(interfaceController: CPInterfaceController) {
        ...
        let t = CPListTemplate(title: tr("Playlists", "Playlists", "播放列表"), sections: [])
    ...
    private func buildList() {
        ...
        if playlists.isEmpty {
            let empty = CPListItem(text: tr("No playlists", "Keine Playlists", "暂无播放列表"), detailText: nil)
    ...
    private func buildOfflineList() {
        ...
        let playlists: [Playlist] = hasDownloads
            ...
            : []
        if playlists.isEmpty {
            let empty = CPListItem(text: tr("No offline playlists", "Keine Offline-Playlists", "暂无离线播放列表"), detailText: nil)
            rootTemplate.updateSections([CPListSection(items: [empty])])
            return
        }
        showPlaylists(playlists)
