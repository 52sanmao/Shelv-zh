...
final class CarPlayLibraryController {
    ...
    init(interfaceController: CPInterfaceController) {
        ...
        let title = isOffline ? tr("Downloads", "Downloads", "下载") : tr("Library", "Bibliothek", "曲库")
    ...
    private func buildMenu() {
        ...
        rootTemplate.tabTitle = isOffline ? tr("Downloads", "Downloads", "下载") : tr("Library", "Bibliothek", "曲库")
        rootTemplate.tabImage = UIImage(systemName: isOffline ? "internaldrive" : "books.vertical")

        var items: [CPListItem] = [
            menuListItem(title: tr("Albums", "Alben", "专辑"), systemImage: "square.stack") { [weak self] _, c in
                c(); self?.pushAlbumsList()
            },
            menuListItem(title: tr("Artists", "Künstler", "艺术家"), systemImage: "music.microphone") { [weak self] _, c in
                c(); self?.pushArtistsList()
            },
        ]
        if UserDefaults.standard.bool(forKey: "enableFavorites") {
            items.append(menuListItem(title: tr("Favorites", "Favoriten", "收藏"), systemImage: "heart") { [weak self] _, c in
    ...
    private func pushAlbumsList() {
        let placeholder = CPListItem(text: tr("Loading…", "Wird geladen…", "加载中…"), detailText: nil)
        let template = CPListTemplate(title: tr("Albums", "Alben", "专辑"), sections: [
    ...
    private func pushArtistsList() {
        let placeholder = CPListItem(text: tr("Loading…", "Wird geladen…", "加载中…"), detailText: nil)
        let template = CPListTemplate(title: tr("Artists", "Künstler", "艺术家"), sections: [
            CPListSection(items: [placeholder], header: nil, sectionIndexTitle: nil)
        ])
    ...
    private func makeArtistSections(_ artists: [Artist], counts: [String: Int]) -> (sections: [CPListSection], itemsByCoverId: [String: [CPListItem]]) {
        ...
        for letter in letters {
            ...
            for artist in (grouped[letter] ?? []).prefix(cap) {
                ...
                let item = artistListItem(artist, subtitle: "\(count) \(tr("albums", "Alben", "张专辑"))") { [weak self] _, c in
                    c()
                    guard let self else { return }
    ...
    private func pushFavorites() {
        ...
        let template = CPListTemplate(title: tr("Favorites", "Favoriten", "收藏"), sections: [])
    ...
    private func makeFavoriteSections(
        ...
        if !songs.isEmpty {
            ...
            if songs.count > kPreviewCount {
                items.append(showAllListItem(title: tr("Show All (\(songs.count))", "Alle anzeigen (\(songs.count))", "显示全部 (\(songs.count))")) { [weak self] _, c in
                    c(); self?.pushFullFavoriteSongs(songs)
                })
            }
            sections.append(CPListSection(items: items, header: tr("Songs", "Titel", "歌曲"), sectionIndexTitle: nil))
        }
        ...
        if !albums.isEmpty {
            ...
            if albums.count > kPreviewCount {
                items.append(showAllListItem(title: tr("Show All (\(albums.count))", "Alle anzeigen (\(albums.count))", "显示全部 (\(albums.count))")) { [weak self] _, c in
                    c(); self?.pushFullFavoriteAlbums(albums)
                })
            }
            sections.append(CPListSection(items: items, header: tr("Albums", "Alben", "专辑"), sectionIndexTitle: nil))
        }
        ...
        if !artists.isEmpty {
            ...
            if artists.count > kPreviewCount {
                items.append(showAllListItem(title: tr("Show All (\(artists.count))", "Alle anzeigen (\(artists.count))", "显示全部 (\(artists.count))")) { [weak self] _, c in
                    c(); self?.pushFullFavoriteArtists(artists)
                })
            }
            sections.append(CPListSection(items: items, header: tr("Artists", "Künstler", "艺术家"), sectionIndexTitle: nil))
        }

        if sections.isEmpty {
            let empty = CPListItem(text: tr("No favorites yet", "Noch keine Favoriten", "暂无收藏"), detailText: nil)
    ...
    private func pushFullFavoriteSongs(_ songs: [Song]) {
        ...
        let template = CPListTemplate(title: tr("Favorite Songs", "Lieblingstitel", "收藏歌曲"), sections: [makeSongsSection()])
    ...
    private func pushFullFavoriteAlbums(_ albums: [Album]) {
        ...
        let template = CPListTemplate(title: tr("Favorite Albums", "Lieblingsalben", "收藏专辑"), sections: [
    ...
    private func pushFullFavoriteArtists(_ artists: [Artist]) {
        ...
        let (items, coverMap) = makeItems()
        let template = CPListTemplate(title: tr("Favorite Artists", "Lieblingskünstler", "收藏艺术家"), sections: [
            CPListSection(items: items, header: nil, sectionIndexTitle: nil)
        ])
        weakTemplate = template
        CarPlayNavigation.safePush(template, on: interfaceController)
